...
government-and-science credible — appropriate for an IndiaAI-aligned food-safety product.
...
credible) for UI, **IBM Plex Mono** for data/coordinates/spectra readouts, **IBM Plex Sans
Devanagari** for Hindi/regional strings (multilingual is a real requirement for Indian field users).
```css
:root {
...
### 5.6 Regional Trends (State Agriculture Dept — anonymized)
- **Purpose:** monitor spray practice & risk hotspots across districts.
...
- **States:** insufficient-data districts shown as no-data, not zero-risk.
...
- **Multilingual:** layout tolerates Hindi/regional strings (Plex Devanagari); never bake text into images.
...
- Plain language for farmers/FPOs; precise/technical for labs and researchers (persona-aware tone).
